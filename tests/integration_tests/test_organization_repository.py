...
async def _seed_organization_data(
    ...
    org_b.phones.append(
        OrganizationPhone(phone="8-923-666-13-13", phone_description="Sales")
    )
...
@pytest.mark.asyncio
async def test_organization_repository_list_by_building(
    async_session: AsyncSession,
) -> None:
    repository = OrganizationRepository(async_session)
    org_a, _ = await _seed_organization_data(async_session)
...
@pytest.mark.asyncio
async def test_organization_repository_list_by_activity(
    async_session: AsyncSession,
) -> None:
    repository = OrganizationRepository(async_session)
    org_a, _ = await _seed_organization_data(async_session)
    food_id = next(
        activity.id for activity in org_a.activities if activity.name == "Food"
    )
...
@pytest.mark.asyncio
async def test_organization_repository_search_by_name(
    async_session: AsyncSession,
) -> None:
    repository = OrganizationRepository(async_session)
...
@pytest.mark.asyncio
async def test_organization_repository_search_by_radius(
    async_session: AsyncSession,
) -> None:
    repository = OrganizationRepository(async_session)
    await _seed_organization_data(async_session)
...
@pytest.mark.asyncio
async def test_organization_repository_search_by_bbox(
    async_session: AsyncSession,
) -> None:
    repository = OrganizationRepository(async_session)
    await _seed_organization_data(async_session)
